...
class billiard_problem:
    def __init__(self,x0=0,y0=-10.0,vx0=1.5,vy0=1.2,dt=0.1):
        self.x=[x0]
        self.y=[y0]
        self.vx=vx0
        self.vy=vy0
    ...
    def calculate(self):
        ...
        for i in range(0,10000):
            ...
            if self.x[-1]**2+self.y[-1]**2<=25:
                self.vx=(2*self.x[-1]*self.y[-1]*self.vy+(self.x[-1]**2+self.y[-1]**2)*self.vx)/(self.y[-1]**2-self.x[-1]**2)
                self.vy=(2*self.x[-1]*self.y[-1]*self.vx+(self.x[-1]**2+self.y[-1]**2)*self.vy)/(-self.y[-1]**2+self.x[-1]**2)
            print self.x[-1]
            print self.y[-1]
            print self.vx
            print self.vy
